...
We first build an application that passes a message from the R server to the JavaScript client to display said message as an vanilla JavaScript alert (pop-up), then have a callback function to know whether the user has clicked 'OK' server-side. Finally we replicate that but with an external library and wrap the whole project into a fully functional package.
