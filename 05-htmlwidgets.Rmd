...
Let's add the option to style the globe, gio.js provides [themes](https://giojs.org/html/docs/colorStyle.html) but they are currently not applicable from R. As a matter of fact, gio.js provides dozens of customisation options that should be integrated into the package. These probably should be split across functions, just like they are in gio.js, rather than all grouped into a single function. This may first appear as difficult to implement, how, when, how would one use another function given the function `gio` generates the visualisation? The answer may surprise, the `gio` function creates everything needed for the visualisation to be generated but does not actually render anything, it returns an object of class "htmlwidget" which renders the visualisation on print (literally `htmlwidget.print` method).
