...
In a sense, while crosstalk establishes lines of communication to transport `key`s between widgets, developers of the respective widgets must handle what `key`s are sent and what to do with incoming `key`s. There are two such lines of communication, one for `key`s of rows to be filtered meant to narrow down the selection of data points displayed on a widget, and another for selection (what crosstalk refers to as "linked brushing") to highlight certain data points.
