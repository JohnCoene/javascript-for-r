...
shared_cars <- SharedData$new(head(cars), group = "cars")
```
...
```{block, type='rmdnote'}
Create the shared dataset the server function or some things might not work as expected in shiny.
```

One can also use the `data` method on the crosstalk object to use in reactive expressions, which is great to use in places where crosstalk is not directly supported, like below in a custom UI block. Note that the argument `withSelection` is set to `TRUE` in order to retrieve the selection state of the rows. 
