...
## Cary Data {#widgets-final-carry-data}

Gio.js only produces one kind of visualisation, a globe with arcs, many libraries you will want to integrate likely will provide multiple types of visualisations (e.g.: scatter and line plots). This generally translates in R packages that mimic ggplot2 by providing geom-like functions where one pipes "geoms" as layers.

For instance echarts4r [@R-echarts4r] provides the following API to draw a scatter plot and line from one dataset.
...
One might wonder how this works internally, well it's surprisingly simple, the data is simply attached to the widget output of the function that initialises the plot. 

```r
mywidget <- function(data) {
