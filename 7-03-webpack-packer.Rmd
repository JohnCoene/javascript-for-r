...
In the previous chapter, a simple shiny application using NPM and webpack was put together. Hopefully, it hinted at some of the powerful things webpack can do but also revealed a downside: the overhead in merely creating the project. Not only these operations have to be repeated for every project, those too: the configuration of webpack used previously will work for other shiny applications but will not for R packages, and golem [@R-golem] applications as the output files and how they are used will drastically change.
...
3. It should not interfere the mission of webpack which is to build more robust JavaScript code. Therefore packer only builds on top of already strict R structures, namely packages (where golem packages can be used to create shiny applications).
...
Whilst htmlwidgets only allow creating scaffolds for widgets packer allows creating scaffolds for various types of projects that involve JavaScript, namely:
...
This gives a few powerful functions that do (almost) all of the job of correctly setting up webpack for your project. These will configure the project as necessary depending on the scaffold you lay down and the context, whether it is a basic package, a golem application, a package with an existing scaffold, etc.

```{block, type='rmdnote'}
One can use multiple scaffolds in a single package or shiny application.
```

Packer goes beyond simply setting up webpack and NPM, it will also create the necessary R functions, roxygen documentation, and examples, so every scaffold is fully functional out-of-the-box!
...
From the root of the package we scaffold a custom input. Notably, this takes a `name` argument, which is used as names for the various files, functions, and modules it creates. The functions print some information about the operations it executes in the package. When run from an interactive session packer also opens the most pertinent files in your default editor or IDE.
...
The scaffold creates the file structure below. Notice that `increment` was used as name of some files and that packer creates three webpack configuration files.

It created one R file, `increment.R` which contains the exported input function which it named `incrementInput`. It also created the `inst/packer` directory which is currently empty but will eventually contain the bundled JavaScript file(s).
...
Finally, the webpack configuration files were created (`webpack.*` files), along with the `srcjs` directory containing core JavaScript files containing the input binding.
...
Above, packer does not use the namespace of functions (e.g. `shiny::tagList`), instead it uses the roxygen2 tags to import the necessary functions: `@importFrom shiny tags tagList`. Rather nicely, packer also created an example in the roxygen documentation.
...
We won't run the example just yet as we first unpack what other files were created before we bundle the JavaScript.
...
The `srcjs/index.js` file was also created, it imports the JavaScript binding detailed above with: `import './inputs/increment.js';`. Importantly though, by default packer does not bundle all of these files into one; `index.js` is only populated for convenience. Instead, packer uses `srcjs/inputs/increment.js` as entry point. It will handle multiple entry points so every input, output, widgets, etc. is bundled separately.

## Bundle {#packer-bundle}

You can then run `packer::bundle` to bundle the JavaScript, the webpack config file includes the correct entry points and output directory. The entry points and output directories will depend on the scaffold, shiny inputs' bundles are placed in the `inst/packer` directory, unless this was run from a golem application, in which case the output is automatically changed to golem's standard.

```r
packer::bundle()
```

By default packer will bundle the files for production, this can be managed with the functions `packer::bundle_dev()` and `packer::bundle_prod()`. Once the JavaScript bundled, we can install or load the package with `devtools::load_all`, and use the example that was created for us to test the input.
...
No code written, yet we have a fully-functional input. We'll leave at this, it's obviously not only meant to create increment buttons but this sets up a solid base for the developer to customise the code and create a different input.
