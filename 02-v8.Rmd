...
Create a package however you usually do, using [usethis](https://usethis.r-lib.org/) [@R-usethis], devtools or the RStudio IDE interface. Below we create a package called "ms" that will hold functionalities we explored in the previous section on npm packages.
...
Now the dependency can be sourced in the `.onLoad` function. We can access the files in the `inst` directory with the `system.file` function. When using the `.onLoad` function it is goo practice to clean up with `.onUnload`.
