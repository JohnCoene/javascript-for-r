...
The Document Object Model comes with events out of the box, though here we use the [convenient API](https://api.jquery.com/category/events/) provided by jQuery. 

```r
library(shiny)

script <- "$('#hoverMe').hover(function(){
  console.log(true);
  Shiny.setInputValue('hovered', true);
})"

ui <- fluidPage(
  h3("Hover the icon below for info."),
  span(
    id = "hoverMe",
    icon("info")
  ),
...
server <- function(input, output) {
  ...
  output$info <- renderUI({
    if(is.null(input$hovered)) 
      return()
    p("This is some information")
  })
