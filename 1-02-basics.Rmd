...
There are two ways to make dependencies: using the htmltools [@R-htmltools] package to create a dependency object that shiny can understand or manually serving and importing the files with shiny.
...
dependency <- htmltools::htmlDependency(
  ...
  stylesheet = c(file = "styles.css")
)
```

There are a few noteworthy things about the above. First, the `src` argument points to a directory, not the file one wants to import. Second, the `script` and `stylesheet` arguments are named vectors that point to files within the `src` directory. One can use the name `file` to point to a local file (as done above) or use `href` to point to a remote file hosted on a CDN.
...
```html
<script src="path/to/directory/script.js"></script>
<link rel="stylesheet" href="path/to/directory/styles.css">
```
