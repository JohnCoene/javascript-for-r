...
Onto creating the R function that will read and print this file. Then again, R is strict, R files must be placed in the `R` directory. Then again, this can be done from the console.
...
It's not done just yet. Let's demonstrate why it is not, it allows introducing devtools. The "test" package can be installed with `devtools::install()`. Running `print_file` returns the error: `could not find function "print_file"`. Odd. This is because we have not explicitly exported the `print_file` function, by default functions and objects declared in `.R` files can only be used internally. To do so we need another package called roxygen2 [@R-roxygen2], it allows writing the documentation in the `.R` file rather than have to create separate file, a lifesaver. Using said documentation we can specify which function should be export with a "tag".
