...
This can be changed by placing a function named `widgetname_html` which is looked up by htmlwidgets and used if found. This function takes the three-dot construct (`...`) and uses them in an htmltools tag. The three-dots are necessary because internally htmlwidgets needs be able to pass the `id`, `class`, and `style` attributes to the tag.
