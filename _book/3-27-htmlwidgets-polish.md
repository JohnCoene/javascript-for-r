...
The above changes allows documentating the input that `gio` accepts more clearly with roxygen2 and also makes its usage more transparent: it is now clear to users what data is required to create a visualisation and they are free to use dataframes of their choice.
...
This small change makes the package a great deal easier to use and understand as `source`, and `target` are vastly clearer to understand than `e` and `i`.
