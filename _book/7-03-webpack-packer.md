...
Packer is comprised of surprisingly few functions; the most important ones are in the `scaffold` family. The term scaffold was borrowed from the htmlwidgets package, which features the function `scaffoldWidget` that was already used in this book. The idea of scaffolds in packer is very similar to the `scaffoldWidget` function: they set up the basic structure for projects.
...
This gives a few powerful functions that do (almost) all of the job of correctly setting up webpack. These will build the necessary file structure and configuration depending on the scaffold you lay down and the context (whether it is a basic package, a golem application, a package with an existing scaffold, etc.)
...
The `srcjs/index.js` file was also created; it imports the JavaScript binding detailed above with `import './inputs/increment.js';`. Importantly, by default packer does not bundle all of these files into one; `index.js` is only populated for convenience in the event one would want to change this behaviour. Instead, packer uses `srcjs/inputs/increment.js` as entry point. It will handle multiple entry points, so every input, output, widgets, etc. are bundled separately. This is done so one can import those dynamically.
...
No code was written, yet we have a fully-functional input! We'll leave at this, it's obviously not only meant to create increment buttons, but this sets up a solid base for the developer to customise the code and conveniently create a different input.

It is worth noting that we built a shiny input from within a package, this is meant to be exported and used in shiny applications elsewhere, but were one to run these exact same steps from a golem application packer would adapt the output path so that the input can be used directly in the application.
