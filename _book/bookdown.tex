...
\begin{verbatim}
## [1] "Sun Jul 12 2020 18:14:03 GMT+0200 (Central European Summer Time)"
\end{verbatim}
...
Consider for instance GeoJSON encoded geographic features, these might not require being loaded into R (converted from JSON to an R data structure), only to be re-serialised to JSON before being read in JavaScript.

Also keep the previous remark in mind when repeatedly serialising data objects, GeoJSON is again a good example. Often, geographic features are not used once but multiple times within a web page (multiple maps); not only should that data not be loaded and serialised but doing so multiple times would be very inefficient given these geographic features do not change: consider providing other ways to for the developer to make that data accessible to the browser.

Below is an example of function that could be used within R markdown or shiny UI to load data in the front-end and bypass serialisation. Note that in addition the function makes use of AJAX (Asynchronous JavaScript And XML) to asynchronously load the data, thereby further reducing load time.
