...
Developing packages used to be notoriously difficult but things have greatly changed namely thanks the devtools and more recent \href{https://usethis.r-lib.org/}{usethis} \citep{R-usethis} packages. The first is specifically designed to help you with creating packages; setting up tests, running checks, building and installing packages, etc. The second, usethis, more broadly helps setting up your project, and automate repetitive tasks. Here, we only skim over the fundamentals, if you want to read more about it look for Hadley Wickham's \href{http://r-pkgs.had.co.nz/}{R Packages} which is freely available on the web.
...
We'll create the following, extremely simple JavaScript file that contains code which declares a variable \texttt{x}.
...
We cab actually create this file from R with:
...
In the \texttt{function.R} file we just created we place a function that reads the \texttt{javascript.js} file and prints it to the R console along with a simple message. To read the file R comes with a \texttt{system.file} function which will return the path the file. Indeed as the package is to be installed on different one can never use absolute paths, it will depend on where the user of your package has it installed. If curious, run \texttt{.libPaths()} to find where it is.
...
We can now add two lines, one read the \texttt{file} and another to print it.
...
Let's add a pretty message, using the \href{https://cli.r-lib.org/}{cli} {[}R-cli{]} package, before we print the file as it allows introducing another concept; using external libraries in our package. To use cli we can run:
...
It does exactly what it printed. The \texttt{DESCRIPTION} file includes information about the package and, very importantly, dependencies. The package ``test'' we are creating now depends on cli. You should now see the following entry in the \texttt{DESCRIPTION} file.
...
It's not done just yet. Let's demonstrate why it is not, it allows introducing devtools. We can build our package with \texttt{devtools::install()}. After running it and typing \texttt{test::} in the console nothing appears. Odd. This is because we have not explicitly exported the \texttt{print\_file} function. To do so we need another package called roxygen2 {[}R-roxygen2{]}, it allows writing the documentation in the \texttt{.R} file rather than have to create separate file, a lifesaver. Using said documentation we can specify which function should be export with a ``tag''.
