...
\begin{verbatim}
## [1] "Sun Jul 12 2020 17:57:30 GMT+0200 (Central European Summer Time)"
\end{verbatim}
...
Remember that data passed to \texttt{htmlwidgets::createWidget} is 1) loaded into R, 2) serialised to JSON, 3) embedded into the HTML output, 4) read back in with JavaScript, which adds some overhead considering it might be read into JavaScript directly. This will not be a problem for most visualisations but might become one when the data is large.

Consider for instance GeoJSON encoded geographic features, these might not require being loaded into R (converted from JSON to an R data structure), to be re-serialised to JSON, only to be read in JavaScript.

Also keep the previous remark in mind when repeatedly serialising data objects, again, GeoJSON can suffer from that. For instance, some GeoJSON encoded features used in multiple places in an application not only should not be loaded and serialised but doing so multiple times would be very inefficient. You might consider providing other ways to load such data.

Below is a mock up function that could be used within the R markdown or shiny UI to load data bypassing any kind of serialisation. Note that in addition the function makes use of AJAX (Asynchronous JavaScript And XML) to asynchronously load the data, thereby further reducing load time.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# this would placed in the shiny UI}
\NormalTok{load\_json\_from\_ui <{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(json)\{}
\NormalTok{  script <{-}}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{"}
\StringTok{    $.ajax(\{ }
\StringTok{        url: \textquotesingle{}"}\NormalTok{, json, }\StringTok{"\textquotesingle{}, }
...
Using the above the data loaded would be accessible from the htmlwidgets JavaScript (e.g.: \texttt{gio.js}) with \texttt{window.globalData}. The \texttt{window} object is akin to the \texttt{document} object, while the latter pertains to the Document Object Model (DOM) and represents the page, the former pertains to the Browser Object Model (BOM) and represents the browser window. So while \texttt{var\ x;} will only be accessible within that script as it's placed in the document (DOM), \texttt{window.x} will accessible anywhere in that window (BOW) and thus anywhere in the document it contains.
