...
One final thing to note for future reference, though not limited to the ids or classes most such selection of elements from the DOM are done with those where the pound sign refers to an element's id (\texttt{\#id}) and a dot relates to an element's class (\texttt{.class}), just like in CSS.
...
For multiple reasons, the best way to include dependencies is probably the former using htmltools. First, it will work with both shiny and rmarkdown \citep{R-rmarkdown} (whereas the other method previousl\textless{} described only works with shiny) reducing the cognitive load on the developer (you); learn to use this method and you will be able to import dependencies for many different output types. Moreover, it comes with neat features that will be explored later in the book, e.g., dynamic dependencies for interactive visualisations or shiny.
...
\textbackslash begin\{center\}\includegraphics[width=1\linewidth]{7-02-webpack-npm-discover_files/figure-latex/unnamed-chunk-4-1}
...
Packer is comprised of surprisingly few functions; the most important ones are in the \texttt{scaffold} family. The term scaffold was borrowed from the htmlwidgets package, which features the function \texttt{scaffoldWidget} that was already used in this book. The idea of scaffolds in packer is very similar to the \texttt{scaffoldWidget} function: they set up the basic structure for projects.
...
\tightlist
\item
  Initialise npm with \texttt{npm\ init\ -y}
\item
...
\item
  Creates basic JavaScript files within the \texttt{srcjs} directory, e.g.: \texttt{index.js}
\item
...
\NormalTok{✔ }\ExtensionTok{Setting}\NormalTok{ active project to }\StringTok{\textquotesingle{}/home/jp/Sites/javascript{-}for{-}r/code/increment\textquotesingle{}}
...
\CommentTok{\#\textquotesingle{} }
\CommentTok{\#\textquotesingle{} ui <{-} fluidPage(}
\CommentTok{\#\textquotesingle{}  incrementInput("theId", 0)}
\CommentTok{\#\textquotesingle{} )}
\CommentTok{\#\textquotesingle{} }
\CommentTok{\#\textquotesingle{} server <{-} function(input, output)\{}
\CommentTok{\#\textquotesingle{} }
...
\AttributeTok{$}\NormalTok{(document).}\AttributeTok{on}\NormalTok{(}\StringTok{"click"}\OperatorTok{,} \StringTok{"button.incrementBinding"}\OperatorTok{,} \KeywordTok{function}\NormalTok{(evt) }\OperatorTok{\{}
...
The \texttt{srcjs/index.js} file was also created; it imports the JavaScript binding detailed above with \texttt{import\ \textquotesingle{}./inputs/increment.js\textquotesingle{};}. Importantly, by default packer does not bundle all of these files into one; \texttt{index.js} is only populated for convenience in the event one would want to change this behaviour. Instead, packer uses \texttt{srcjs/inputs/increment.js} as entry point. It will handle multiple entry points, so every input, output, widgets, etc. are bundled separately. This is done so one can import those dynamically.
...
\chapter{Packer Advanced}\label{packer-adv}}

We're about to cover slightly more advanced uses of NPM and webpack with R using packer. These involve using an NPM dependency to develop a widget, and use \href{https://vuejs.org/}{Vue.js} to power the front-end of a shiny application.
...
This indeed displays the message in \texttt{\textless{}h1\textgreater{}} HTML tags. Now onto unpacking the structure generated by packer. We'll skip the R code to keep this concise as nothing differs from a standard widget on that side. Rather, we'll focus on the JavaScript code in the \texttt{srcjs} directory. First, in the \texttt{srcjs/widgets} directory one will find the file \texttt{countup.js}, this file contains the code that produces the widget.
...
The \texttt{header.js} file includes the \texttt{asHeader} function which accepts an \texttt{x} argument that is used to create the \texttt{\textless{}h1\textgreater{}} message. This functions is exported.
...
We will make changes to the JavaScript so that instead of displaying the message as text, uses the aforementioned countup library to animate a number. The first order of business is to in install countup. Here we use packer to do so, the function call below is identical to running \texttt{npm\ install\ countup\ -\/-save} from the terminal.
...
\section{Shiny with Vue}\label{packer-adv-shiny-vue}}
