...
\item
  The book opens with an introduction to better understand its premise. It starts with rationales for using JavaScript in conjunction with R which are illustrated with existing R packages that make use of JavaScript. Then the book briefly describes concepts essential to understand before proceeding. Finally, the various methods with which one might make JavaScript work with R are listed.
\item
  Then the book delves into using JavaScript for computations, namely via the V8 engine and node.js. After a short introduction, chapters will walk the reader through various examples: a fuzzy search, a time format converter, and a basic natural language tokenizer.
\item
  The third part of the book details how JavaScript can work with Shiny. Once the basics out of the way, we build utilities to display notifications which are later improved using an external library. Then we create a functionality that displays when the shiny server is busy running computations. The part ends with an example of custom shiny output for a library that applies filters to images.
\item
  The largest part of the book concerns data visualisation, the book delves into creating interactive outputs with the htmlwidgets and shiny packages. This part opens with a brief overview of how it works and libraries that make great candidates for the htmlwidgets package. Then a first widget is built, this one is rather unimpressive but immensely useful to look under the hood of such outputs and grasp a better understanding of they work. Then a second package is built, one that allows to mimic text being typed. Next, we tackle a more substantial library to draw arcs between countries on a 3D globe. The last two chapters of that part of the book delves into more advanced topics and how such interactive visualisations can work with Shiny.
\item
  Finally the book closes with examples of all the integrations explored previously. This involves recreating (a part of) the plotly package, building an image classifier, adding progress bars to a shiny application, building a math evaluator and running basic machine learning operations.
\end{enumerate}
...
\begin{verbatim}
## [1] "Tue Jul 07 2020 18:16:26 GMT+0200 (Central European Summer Time)"
\end{verbatim}
