...
\begin{verbatim}
## [1] "Sat Jul 11 2020 21:26:48 GMT+0200 (Central European Summer Time)"
\end{verbatim}
...
Because jQuery is already imported there is no need to do so, on the contrary, it might cause clashes. Below we create a shiny application containing a message handler to toggle (switch between show hide element depending on state), this is triggered by a button.
...
\StringTok{        $(\textquotesingle{}\#\textquotesingle{} + id).toggle();}
\StringTok{      \});"}
\NormalTok{    )}
\NormalTok{  ),}
...
\NormalTok{    session}\OperatorTok{$}\KeywordTok{sendCustomMessage}\NormalTok{(}\StringTok{\textquotesingle{}jquery{-}toggle\textquotesingle{}}\NormalTok{, }\DataTypeTok{msg =} \StringTok{"text"}\NormalTok{)}
\NormalTok{  \})}
