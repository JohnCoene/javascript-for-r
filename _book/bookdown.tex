...
Therefore, the code below creates two shared datasets that are linked as they fall in the same \texttt{group} despite the fact that they are separate R objects.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# row.name will be used as keys}
\KeywordTok{row.names}\NormalTok{(mtcars)[}\DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{]}
...
This effectively means that the key \texttt{Mazda\ RX4} ``means'' the same thing in \texttt{shared\_cars} and \texttt{shared\_cars\_head}.
...
The \texttt{origData} method is needed to extract the data from the shared dataset. This is of course necessary as the \texttt{gio} function still needs to obtain and serialise the arcs to display on the globe.
...
\begin{verbatim}
## [1] "Wed Aug 26 2020 23:06:00 GMT+0200 (Central European Summer Time)"
\end{verbatim}
