...
Tags are preceded by a special comment \texttt{\#\textquotesingle{}} and the tags themselves are defined with \texttt{@}, there are plenty of them, we don't explore any other in this book.
...
Now one can run \texttt{devtools::document()} to generate the documentation of the package (the \texttt{.Rd} files) using the export tag we added: this exports the function by adding \texttt{export(print\_file)} to the \texttt{NAMESPACE} file, which you should not edit yourself. Then we can run \texttt{devtools::install()} to build and install the package.
...
\begin{figure}
\centering
\includegraphics{images/javascript-intro.png}
\caption{Printing to the console}
\end{figure}
...
\includegraphics{bookdown_files/figure-latex/unnamed-chunk-15-1.png}
...
\includegraphics{bookdown_files/figure-latex/unnamed-chunk-16-1.png}
...
\begin{verbatim}
## [1] "Mon Aug 03 2020 14:24:10 GMT+0200 (Central European Summer Time)"
\end{verbatim}
...
\KeywordTok{\textless{}html\textgreater{}}
  \KeywordTok{\textless{}head\textgreater{}}
    \KeywordTok{\textless{}script}\OtherTok{ src=}\StringTok{\textquotesingle{}https://cdnjs.cloudflare.com/.../fuse.min.js\textquotesingle{}}\KeywordTok{\textgreater{}\textless{}/script\textgreater{}}
  \KeywordTok{\textless{}/head\textgreater{}}
  \KeywordTok{\textless{}body\textgreater{}}
  \KeywordTok{\textless{}/body\textgreater{}}
\KeywordTok{\textless{}/html\textgreater{}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n}\OperatorTok{$}\KeywordTok{eval}\NormalTok{(}\StringTok{"var words = tokenizer.tokenize(\textquotesingle{}Using nodejs.\textquotesingle{})"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n}\OperatorTok{$}\KeywordTok{eval}\NormalTok{(}
  \StringTok{"var analyzer = new Analyzer(\textquotesingle{}English\textquotesingle{}, stemmer, \textquotesingle{}afinn\textquotesingle{});"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{server \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(input, output, session)\{}
\NormalTok{  session}\OperatorTok{$}\KeywordTok{sendCustomMessage}\NormalTok{(}\DataTypeTok{type =} \StringTok{"send{-}alert"}\NormalTok{, }\DataTypeTok{message =} \StringTok{"This is not used"}\NormalTok{)}
\NormalTok{\}}
