...
\begin{verbatim}
## [1] "Mon Jun 01 2020 13:58:18 GMT+0200 (CEST)"
\end{verbatim}
...
Before we move on to other things one should also grasp a better understanding of the \texttt{el} object. In the widget previously created
...
To explore this further, and in a more interactive context re-run the code below to create a widget, open it in the browser (\includegraphics{images/open-in-browser.png}) right-click to select ``Inspect.''
...
This displays the HTML element created by htmlwidgets and meant to hold the visualisation, or in this case, the message. If you are familiar with JavaScript, this is the element that would be returned by \texttt{document.getElementById}. In attempt to become more at ease with this setup let us change something at have play with the widget.

Out-of-the-box htmlwidgets uses \texttt{innerText}, which does very much what it says on the tin, it places takes inside an element. JavaScript comes with another function akin to \texttt{innerText}, \texttt{innerHTML}, while the former only allows inserting text the former lets one insert any HTML.
