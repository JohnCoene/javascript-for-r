...
\begin{verbatim}
## [1] "Wed Jul 01 2020 22:18:45 GMT+0200 (Central European Summer Time)"
\end{verbatim}
...
\NormalTok{  output}\OperatorTok{$}\NormalTok{table <{-}}\StringTok{ }\KeywordTok{renderDT}\NormalTok{(\{}
    \KeywordTok{datatable}\NormalTok{(mtcars, }\DataTypeTok{escape =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{selection =} \StringTok{"none"}\NormalTok{, }\DataTypeTok{rownames =} \OtherTok{FALSE}\NormalTok{)}
\NormalTok{  \})}
...
We start by creating the on-click functions (character strings) for each row of the \texttt{mtcars} dataset. This is the function that will be triggered when buttons are clicked.
