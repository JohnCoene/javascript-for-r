...
Importantly, the focus of the book truly is the integration of JavaScript with R where both languages actively interact with one another, it is not merely about including JavaScript code that woks alongside R.
...
\item
  The book opens with an introduction to better understand its premise. It starts with rationales for using JavaScript in conjunction with R which are supported with existing R packages that already make use of JavaScript and are available on CRAN. Then we briefly describe concepts essential to understand the rest of the book to ensure the reader can follow along. Finally, the chapter closes by listing the various methods with which one might make JavaScript work with R.
\item
  Then we delve into using JavaScript for computations, namely via the V8 engine and node.js. After a short introduction, chapters will walk the reader through various examples: a fuzzy search, a time format converter, and basic natural language operations.
\item
  The third part of the book details how JavaScript can work with Shiny. Once the basics out of the way, we build utilities to display notifications. Then we create a functionality to notify the user when the shiny server is busy running computations. This part of the book ends with an example of custom shiny output for a library that applies filters to images.
\item
  The largest part of the book concerns data visualisation, it plunges into creating interactive outputs with the htmlwidgets and shiny packages. This opens with a brief overview of how it works and libraries that make great candidates to integrate with the htmlwidgets package. Then a first widget is built, a rather unimpressive one, but it immensely useful to look under the hood of such outputs to observe its inner-workings and grasp a better understanding of how htmlwidgets work. Then a second package is built, one that allows to mimic text being typed. Next, we tackle a more substantial library that allows drawing arcs between countries on a 3D globe. The last two chapters go into more advanced topics such as security and resizing as well as how such interactive visualisations can work hand in hand with the shiny framework.
\item
  Finally the book closes with examples of all the integrations explored previously. This involves recreating (a part of) the plotly package, building an image classifier, adding progress bars to a shiny application, building a math evaluator and running basic machine learning operations in JavaScript.
\end{enumerate}
...
\begin{verbatim}
## [1] "Fri Jul 10 2020 17:58:14 GMT+0200 (Central European Summer Time)"
\end{verbatim}
...
\KeywordTok{const}\NormalTok{ fuse }\OperatorTok{=} \KeywordTok{new}\NormalTok{ Fuse(list}\OperatorTok{,}\NormalTok{ options)}
...
\KeywordTok{\textless{}html\textgreater{}}
  ...
  \KeywordTok{\textless{}/head\textgreater{}}
  \KeywordTok{\textless{}body\textgreater{}}
    \KeywordTok{\textless{}p\textgreater{}}\NormalTok{Content}\KeywordTok{\textless{}/p\textgreater{}}
  \KeywordTok{\textless{}/body\textgreater{}}
\KeywordTok{\textless{}/html\textgreater{}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{npm \textless{}{-}}\StringTok{ }\NormalTok{Npm}\OperatorTok{$}\KeywordTok{new}\NormalTok{()}\OperatorTok{$}\KeywordTok{install}\NormalTok{(}\StringTok{"natural"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\textquotesingle{} Create an Alert}
\CommentTok{\#\textquotesingle{} @export}
\NormalTok{send\_alert \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(}\DataTypeTok{content =} \StringTok{"alert"}\NormalTok{, }\DataTypeTok{color =} \StringTok{"blue"}\NormalTok{, }\DataTypeTok{session =}\NormalTok{ shiny}\OperatorTok{::}\KeywordTok{getDefaultReactiveDomain}\NormalTok{())\{}
  \CommentTok{\# define notice options}
\NormalTok{  notice =}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{content =}\NormalTok{ content, }\DataTypeTok{color =} \StringTok{"black"}\NormalTok{)}
...
\CommentTok{\#\textquotesingle{} Create an Alert}
\CommentTok{\#\textquotesingle{} @export}
\NormalTok{send\_alert \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(id, }\DataTypeTok{content =} \StringTok{"alert"}\NormalTok{, }\DataTypeTok{color =} \StringTok{"blue"}\NormalTok{, }\DataTypeTok{session =}\NormalTok{ shiny}\OperatorTok{::}\KeywordTok{getDefaultReactiveDomain}\NormalTok{())\{}
  \CommentTok{\# define notice options}
\NormalTok{  notice =}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{content =}\NormalTok{ content, }\DataTypeTok{color =} \StringTok{"black"}\NormalTok{)}
...
\NormalTok{  output}\OperatorTok{$}\NormalTok{table \textless{}{-}}\StringTok{ }\KeywordTok{renderDT}\NormalTok{(\{}
\NormalTok{    onclick \textless{}{-}}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{"Shiny.setInputValue(\textquotesingle{}click\textquotesingle{}, \textquotesingle{}"}\NormalTok{, }\KeywordTok{rownames}\NormalTok{(mtcars), }\StringTok{"\textquotesingle{})"}\NormalTok{)}
    \KeywordTok{datatable}\NormalTok{(}
\NormalTok{      mtcars, }
      \DataTypeTok{escape =} \OtherTok{FALSE}\NormalTok{, }
      \DataTypeTok{selection =} \StringTok{"none"}\NormalTok{, }
...
\NormalTok{  output}\OperatorTok{$}\NormalTok{table \textless{}{-}}\StringTok{ }\KeywordTok{renderDT}\NormalTok{(\{}
\NormalTok{    onclick \textless{}{-}}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{"Shiny.setInputValue(\textquotesingle{}click\textquotesingle{}, \textquotesingle{}"}\NormalTok{, }\KeywordTok{rownames}\NormalTok{(mtcars), }\StringTok{"\textquotesingle{})"}\NormalTok{)}
\NormalTok{    button \textless{}{-}}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{"\textless{}a class=\textquotesingle{}btn btn{-}primary\textquotesingle{} onClick=}\CharTok{\textbackslash{}"}\StringTok{"}\NormalTok{, onclick, }\StringTok{"}\CharTok{\textbackslash{}"}\StringTok{\textgreater{}Click me\textless{}/a\textgreater{}"}\NormalTok{)}
...
\begin{Highlighting}[]
\CommentTok{\#\textquotesingle{} @export}
\NormalTok{lenaOutput \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(id)\{}
...
\CommentTok{\#\textquotesingle{} @export}
\NormalTok{lenaOutput \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(id)\{}
\NormalTok{  el \textless{}{-}}\StringTok{ }\NormalTok{shiny}\OperatorTok{::}\NormalTok{tags}\OperatorTok{$}\KeywordTok{canvas}\NormalTok{(}\DataTypeTok{id =}\NormalTok{ id, }\DataTypeTok{class =} \StringTok{"lena"}\NormalTok{)}

\NormalTok{  path \textless{}{-}}\StringTok{ }\KeywordTok{system.file}\NormalTok{(}\StringTok{"assets"}\NormalTok{, }\DataTypeTok{package =} \StringTok{"lena"}\NormalTok{)}
...
\NormalTok{    $(}\BuiltInTok{document}\NormalTok{)}\OperatorTok{.}\FunctionTok{ready}\NormalTok{(}\KeywordTok{function}\NormalTok{()\{}
      \CommentTok{// Get the image}
      \KeywordTok{var}\NormalTok{ originalImage }\OperatorTok{=} \BuiltInTok{document}\OperatorTok{.}\FunctionTok{getElementById}\NormalTok{(data}\OperatorTok{.}\AttributeTok{img\_id}\NormalTok{)}\OperatorTok{;}

      \CommentTok{// Filter to apply, in this case the red filter}
      \KeywordTok{var}\NormalTok{ filter }\OperatorTok{=}\NormalTok{ LenaJS[data}\OperatorTok{.}\AttributeTok{filter}\NormalTok{]}\OperatorTok{;}

      \CommentTok{// Apply the filter}
\NormalTok{      LenaJS}\OperatorTok{.}\FunctionTok{filterImage}\NormalTok{(el}\OperatorTok{,}\NormalTok{ filter}\OperatorTok{,}\NormalTok{ originalImage)}\OperatorTok{;}
\NormalTok{    \})}\OperatorTok{;}
...
\NormalTok{Shiny}\OperatorTok{.}\AttributeTok{outputBindings}\OperatorTok{.}\FunctionTok{register}\NormalTok{(lenaBinding}\OperatorTok{,} \StringTok{"lenaPackage.lenaBinding"}\NormalTok{)}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  output}\OperatorTok{$}\NormalTok{filtered \textless{}{-}}\StringTok{ }\KeywordTok{renderLena}\NormalTok{(\{}
  ...
  \DataTypeTok{factory}\OperatorTok{:} \KeywordTok{function}\NormalTok{(el}\OperatorTok{,}\NormalTok{ width}\OperatorTok{,}\NormalTok{ height) \{}
    ...
    \ControlFlowTok{return}\NormalTok{ \{}
      ...
      \DataTypeTok{renderValue}\OperatorTok{:} \KeywordTok{function}\NormalTok{(x) \{}
        ...
        \KeywordTok{var}\NormalTok{ container }\OperatorTok{=} \BuiltInTok{document}\OperatorTok{.}\FunctionTok{getElementById}\NormalTok{(el}\OperatorTok{.}\AttributeTok{id}\NormalTok{)}\OperatorTok{;}
\NormalTok{        controller }\OperatorTok{=} \KeywordTok{new}\NormalTok{ GIO}\OperatorTok{.}\FunctionTok{Controller}\NormalTok{(container)}\OperatorTok{;} \CommentTok{// declared outside}
...
\CommentTok{// gio.js}
\KeywordTok{var}\NormalTok{ container }\OperatorTok{=} \BuiltInTok{document}\OperatorTok{.}\FunctionTok{getElementById}\NormalTok{(el}\OperatorTok{.}\AttributeTok{id}\NormalTok{)}\OperatorTok{;}
\NormalTok{container}\OperatorTok{.}\AttributeTok{innerHTML} \OperatorTok{=} \StringTok{\textquotesingle{}\textquotesingle{}}\OperatorTok{;} \CommentTok{// empty the container}
\NormalTok{controller }\OperatorTok{=} \KeywordTok{new}\NormalTok{ GIO}\OperatorTok{.}\FunctionTok{Controller}\NormalTok{(container)}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#\textquotesingle{} @export}
\NormalTok{gio\_send\_data \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(id, data, }\DataTypeTok{session =}\NormalTok{ shiny}\OperatorTok{::}\KeywordTok{getDefaultReactiveDomain}\NormalTok{())\{}
  ...
  \DataTypeTok{factory}\OperatorTok{:} \KeywordTok{function}\NormalTok{(el}\OperatorTok{,}\NormalTok{ width}\OperatorTok{,}\NormalTok{ height) \{}
    ...
    \ControlFlowTok{return}\NormalTok{ \{}
      ...
      \DataTypeTok{renderValue}\OperatorTok{:} \KeywordTok{function}\NormalTok{(x) \{}
        ...
        \KeywordTok{var}\NormalTok{ container }\OperatorTok{=} \BuiltInTok{document}\OperatorTok{.}\FunctionTok{getElementById}\NormalTok{(el}\OperatorTok{.}\AttributeTok{id}\NormalTok{)}\OperatorTok{;}
\NormalTok{        container}\OperatorTok{.}\AttributeTok{innerHTML} \OperatorTok{=} \StringTok{\textquotesingle{}\textquotesingle{}}\OperatorTok{;}
\NormalTok{        controller }\OperatorTok{=} \KeywordTok{new}\NormalTok{ GIO}\OperatorTok{.}\FunctionTok{Controller}\NormalTok{(container)}\OperatorTok{;}
...
\begin{Highlighting}[]
\CommentTok{\#\textquotesingle{} @export}
\NormalTok{plotlyProxy \textless{}{-}}\StringTok{ }\ControlFlowTok{function}\NormalTok{(id, }\DataTypeTok{session =}\NormalTok{ shiny}\OperatorTok{::}\KeywordTok{getDefaultReactiveDomain}\NormalTok{())\{}
  \KeywordTok{list}\NormalTok{(}
...
\NormalTok{  code \textless{}{-}}\StringTok{ }\KeywordTok{paste0}\NormalTok{(address, }\StringTok{" = new ML.SimpleLinearRegression(x, y);"}\NormalTok{)}
...
The application will prompt the user to input their name, this will be stored in a cookie so that on their next visit they are welcomed to the app with a personalised message. Cookies are natively supported by web browsers and JavaScript, there is, however, a great library which eases greatly how those cookies are handled: \href{https://github.com/js-cookie/js-cookie}{js-cookie}.

\hypertarget{discover-js-cookie}{%
\section*{Discover js-cookie}\label{discover-js-cookie}}


The library is at its core very straightforward, it exports a \texttt{Cookie} object from which one can access the \texttt{set}, \texttt{get}, and \texttt{remove} methods. There is also the possibility to pass additional options when defining the cookie, such as how long it is valid for, but we won't explore these here.
...
First we define a JavaScript function that retrieves the cookies that it sets as \texttt{cookies} input. The reason we do so is simply because we will have to execute this in multiple places, this will become clearer in just a second.
...
Then we define the message handlers, we define two distinct such handlers, one that sets the cookie and another that removes it. Note that both of then run the \texttt{getCookies} function defined previously. The reason this is done is that we need the input to be updated with the new values after it is set and after it is removed. Otherwise setting or removing the cookie will leave the actual input value untouched.
...
\begin{figure}
\centering
\includegraphics{images/shiny-cookies.png}
