...
\begin{verbatim}
## [1] "Sun Jun 07 2020 16:24:40 GMT+0200 (CEST)"
\end{verbatim}
...
\subsection*{Deserialise}\label{deserialise}}


In the section on sending data from R to JavaScript we used a ``message handler'' in JavaScript to handle the data coming from the server. The corollary exists, we can create an ``input handler'' to preprocess the data coming from JavaScript before it is made accessible by the input. In R, we create a function that accepts three arguments, the data coming JavaScript, a Shiny session, and the name of the input. Note that all of these arguments are mandatory even if we are not going to used them.

To demonstrate, we will create a handler that adds some admittedly useless meta information to the data.
...
Once this function created it needs to be registered with shiny using the \texttt{registerInputHandler} function which takes two arguments. First, the name of the handler which will need to be referenced in JavaScript inputs which should use the handler, second, the handler function. Attempt to give the handler a unique name yet simple to avoid clashes with other handlers.
...
Note that handlers can only be registered once, running the above twice will fail the second time, even if the handler function has changed. This can be overwritten using the \texttt{force} argument.
...
The previous section on the input handler is entirely optional but as we will see in later parts of the book it can prove nice touch in some cases.
...
\NormalTok{resize}\OperatorTok{:} \KeywordTok{function}\NormalTok{(width}\OperatorTok{,}\NormalTok{ height) }\OperatorTok{\{}
  \VariableTok{chart}\NormalTok{.}\AttributeTok{setSize}\NormalTok{(width}\OperatorTok{,}\NormalTok{ height)}\OperatorTok{;} \CommentTok{// highcharts example}
\OperatorTok{\}}
\NormalTok{...}
\end{Highlighting}
\end{Shaded}
...
Nonetheless, it is doable with htmlwdigets' serialiser (an only that one). The function \texttt{htmlwidgets::JS} can be used to mark a character vector so that it will be treated as literal JavaScript code when evaluated on the client-side.
