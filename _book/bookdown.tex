...
\begin{verbatim}
## [1] "Mon Jun 01 2020 22:21:29 GMT+0200 (CEST)"
\end{verbatim}
...
This displays the message in the RStudio ``Viewer,'' or the your default browser which indicates that the function creates an HTML output, the \includegraphics{images/open-in-browser.png} button located in the top right of the RStudio ``Viewer'' will open the message in your default browser. Opening the output in the browser can prove very useful to look under the hood of the widgets for debugging.
...
\tightlist
\item
  By opening the visualisation in your browser using the \includegraphics{images/open-in-browser.png} button located in the top right of the ``Viewer,'' and in the browser right clicking on the message to select ``Inspect,''
\item
...
The first thing the source code reveals is the element (\texttt{div}) created by the htmlwidgets package to hold the message, as well as the JSON object which contains, amongst other things, the \texttt{x} variable used in the \texttt{play} function.
...
This produces an HTML element with the specified \texttt{elementId} as HTML \texttt{id} in the \texttt{div}.
...
You will also notice that this affect the \texttt{script} tag below, the \texttt{data-for} attribute of which is also set to ``myViz,'' this tells the script which widget the JSON it contains is to be used for.
...
This use of id may be not obvious at first, but one needs to be able to handle multiple widgets in a single R markdown document or shiny application, distinguishing between them and their respective data inputs is thus crucial. Inspecting the output also shows the dependencies imported, these are placed within the \texttt{head} HTML tags at the top of the page.
...
This effectively imports the \texttt{htmlwidgets.js} library as well as the \texttt{play.js} file, and were the visualisation depending on external libraries they would appear alongside those. Peaking inside the \texttt{play.js} file located at \texttt{inst/htmlwidgets/play.js} reveals the code below.
...
We place \texttt{console.log} to print the content of \texttt{x} in the console, reload the package functions with \texttt{devtools::load\_all} and use the function \texttt{play} again then explore the console from the browser.
...
This displays the JSON object containing the message which eerily resembles the list created in R (\texttt{x\ =\ list(message\ =\ "This\ is\ a\ widget!")}). What one should take away from this is that data that needs to be communicated from R to the JavaScript function should be placed in the R list \texttt{x}.

Before we move on to other things one should also grasp a better understanding of the \texttt{el} object, which can also be logged.
...
In attempt to become more at ease with this setup let us change something and play with the widget. Out-of-the-box htmlwidgets uses \texttt{innerText}, which does very much what it says on the tin, it places takes inside an element. JavaScript comes with another function akin to \texttt{innerText}, \texttt{innerHTML}, while the former only allows inserting text the former lets one insert any HTML.
...
\KeywordTok{as.character}\NormalTok{(shiny}\OperatorTok{::}\KeywordTok{h1}\NormalTok{(}\StringTok{"HTML tag"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Let's incorporate it in our package by replacing the content of the \texttt{renderValue} in \texttt{typed.js} with the above.
...
This, however, will cause issues as the \texttt{strings} options expects and array (a vector) and not a single string.
...
For instance, renaming the \texttt{message} to \texttt{strings} effectively does this.
...
The full list of which is available in the \href{https://github.com/mattboldt/typed.js/\#customization}{documentation of typed.js}.
