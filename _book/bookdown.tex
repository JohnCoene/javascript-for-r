...
\begin{verbatim}
## [1] "Sat Jul 04 2020 18:23:15 GMT+0200 (Central European Summer Time)"
\end{verbatim}
...
First, we create the directories and file necessary, we'll be using a rather well-known gif in the R community to indicate the server is busy.
...
Then we create an application that draws and redraws a plot at the click of a button.
...
\NormalTok{  tags}\OperatorTok{$}\KeywordTok{script}\NormalTok{(}\DataTypeTok{src =} \StringTok{"www/script.js"}\NormalTok{),}
  \KeywordTok{h1}\NormalTok{(}\StringTok{"Is is recalculating?"}\NormalTok{, }\DataTypeTok{id =} \StringTok{"color"}\NormalTok{),}
...
The app displays the gif but it should only be visible when the server is busy, this can be solved with CSS. The code below hides the gif with \texttt{visibility:\ hidden}, and repositions it, floating on top of the rest of the content in the top right of the page.
...
We can then use the shiny events to dynamically show and hide the gif when the user is busy. Below we use observe the event \texttt{shiny:busy} on the entire page, when the event is triggered retrieve the gif and make it visible by changing the CSS \texttt{visibility} property.
