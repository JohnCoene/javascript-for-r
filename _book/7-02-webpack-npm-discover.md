...
Rather interestingly, because multiple variables can be declared on a single line (e.g., `var a,b,c;`) but only a single default can exist, the default export\index{export} and vriable declaration cannot be placed on a single line. 
